...
export default function AdminMDAsPage() {
  ...
  useEffect(() => {
    const fetchMDAs = async () => {
      try {
        const mdasData = await apiService.getMDAs();
        setMdas(mdasData);
      } catch (error) {
        console.log(error);
      } finally {
  ...
  const handleDelete = async (mdaId: string | number) => {
    if (!window.confirm("Are you sure you want to delete this MDA?")) return;

    try {
      await apiService.deleteMDA(mdaId);
      setMdas(mdas.filter((mda) => mda.id !== mdaId));
    } catch (error) {
      console.log(error);
    }
  };
