...
const MDADetail = () => {
  ...
  useEffect(() => {
    const fetchMDA = async () => {
      ...
      try {
        ...
        setMda(data);
        // Notes: Comments for MDAs would need to be implemented if required
        // const mdaComments = await apiService.getComments({ mdaId: id });
        // setComments(mdaComments);
      } catch (error) {
        console.log(error);
        toast.error("Failed to load MDA", { autoClose: 3000 });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="container mx-auto px-4 py-8 font-sans">
      ...
      />
      <SocialShareButtons
        url={`${window.location.origin}/mda/${mda.id}`}
        title={mda.name}
